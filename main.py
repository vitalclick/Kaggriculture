...
Giving each unit its own region was tried as an alternative and is decisively
worse -- per-turn clustering, a fixed grid, and clusters frozen for a day all
landed $53k-$68k below this. Weighting travel instead gets the locality (59% of
actions are movement, down from 64%) without giving up the ability to swarm.
"""
...
# How hard distance counts against a job's value when routing. A job worth V
# that is d steps away scores V / (1+d)**TRAVEL_PENALTY. At 1.0 -- plain value
# per action -- hands chase the richest job anywhere on the map and two thirds
# of every action is walking. Penalising travel harder keeps them working a
...
def _plan(obs, cfg=None):
    ...
    # Greedy over value per action, with travel weighted by TRAVEL_PENALTY.
    # Giving each unit its own region of the farm was tried instead and is
    # clearly worse (three variants, all $53k-$68k below this): work is bursty
    # and unevenly spread, so a fixed patch idles a hand whose ground is quiet
    # while a block ripening next door goes unpicked, and every unit still has
    # to cross to the shed to drop and sell. A soft penalty buys the locality
    # without giving up the crew's ability to swarm.
    pairs = []
    for i in range(n_units):
        ...
        for k, t in enumerate(tasks):
            if t["need"] and invs[i].get(t["need"], 0) <= 0:
                continue
            pairs.append((-t["v"] / (1.0 + dist(units[i], t["pos"])) ** TRAVEL_PENALTY, i, k))
    pairs.sort()
